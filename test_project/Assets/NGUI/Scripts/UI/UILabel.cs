...
public class UILabel : UIWidget
{
    ...
    [System.NonSerialized] bool mInitSetOnce = false;

    //haru - 네오엠엘비 전용 코드: 디폴트 폰트 사용
    [HideInInspector] [SerializeField] public bool mUseDefaultFont = false;

    public bool UseDefaultFont
    {
        get { return mUseDefaultFont; }
        set { mUseDefaultFont = value; }
    }
    //haru - 여기까지
    ...
    public string processedText
    {
        get
        {
            ...
            //haru - NGUI Clamp 관련 오류로 랜덤으로 텍스트가 다음줄로 넘어가는 상황발생
            //ProcessText를 더하면 괜찮아지는것을 발견하고 아마도 ProcessText호출시점과 렌더링 시점이 겹칠때 문제인걸로 확인됨. 
            //UILabel을 더깊게 파는건 의미가 없을듯 하여, Clamp모드일때는 shouldBeProcessed를 무조건 true로 설정하여 매번 ProcessText를 호출하도록 함.
            //퍼포먼스 문제가 걱정되었으나 Update에서 매프레임마다 호출하지 않는것을 확인함.
            if (mOverflow == Overflow.ClampContent && mOverflowEllipsis == true)
            {
                mShouldBeProcessed = true;
            }
    ...
    //haru - 네오엠엘비 전용 코드: 언어에 따라서 폰트 적용. OnStart() 에서 호출시 화면 전환때 폰트 깨지는 현상발생 Awake()에서 호출 하자. 
    void ChangeLocalFont()
    {
        if (trueTypeFont != null)
        {
            //폰트이름을 통해서 bold 판단
            bool isBold = trueTypeFont.name.Substring(trueTypeFont.name.Length - 4, 4).Equals("Bold");

            if (mUseDefaultFont)
            {
                trueTypeFont = isBold ? Resources.Load("NotoSansKR-Bold") as Font : Resources.Load("NotoSansKR-Regular") as Font;
            }
            else
            {
                //언어에 따라서 해당 폰트로 변경
                ...
                //{
                //    switch (LanguageManager.Instance.CurrentLangType)
                //    {
                //        case SystemLanguage.English:
                //        case SystemLanguage.Spanish:
                //            trueTypeFont = isBold ? Resources.Load("RobotoCondensed-Bold") as Font : Resources.Load("RobotoCondensed-Regular") as Font;

                //            // //영문 폰트만 5% 크게!
                //            // this.fontSize = this.fontSize + 1;// * 105 / 100;
                //            break;
                //        case SystemLanguage.Chinese:
                //        case SystemLanguage.ChineseSimplified:
                //        case SystemLanguage.ChineseTraditional:
                //            trueTypeFont = isBold ? Resources.Load("NotoSansTC-Bold") as Font : Resources.Load("NotoSansTC-Regular") as Font;
                //            break;
                //        case SystemLanguage.Japanese:
                //            trueTypeFont = isBold ? Resources.Load("NotoSansJP-Bold") as Font : Resources.Load("NotoSansJP-Regular") as Font;
                //            break;
                //        default:
                //            trueTypeFont = isBold ? Resources.Load("NotoSansKR-Bold") as Font : Resources.Load("NotoSansKR-Regular") as Font;
                //            break;
    ...
    protected override void OnStart()
    {
        ...
        if (!mInitSetOnce)
        {
            //Debug.LogWarning("Check Start mInitSetOnce");
            mInitSetOnce = true;
            //AceLabelParser _parser = gameObject.GetComponent<AceLabelParser>();
            //if (_parser != null)
            //{
            //    _parser.enabled = true;
            //}
        }
